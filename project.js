//This was a Graphics Project done with another student, Bev Raudry
//We each created different objects in the scene, then merged our scenes together
...
window.onload = function init() {
	...
	openNewTexture('assets/textures/wall.jpg');
	openNewTexture('assets/textures/wrapping.jpg');
	openNewTexture('assets/textures/rug.jpg');
...
function drawRug(x,y,z) {
  materialAmbient = vec4( .3, .3, .4, 1.0 );
  materialDiffuse = vec4( .5, .5, .5, 1.0);
  materialSpecular = vec4( .3, .3, .3, 1 );
  materialShininess = 50.0;
  var materialShininess = 10.0;
...
audio.src = 'assets/music/winterwonder.mp3';
audio.currentTime = 0;
